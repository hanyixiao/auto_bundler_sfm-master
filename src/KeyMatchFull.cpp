...
int main(int argc, char **argv) {
    ...
    if (argc != 3 && argc != 4) {
		...
		printf("changing the code,han 2019-2-17-0 \r\n");
        return EXIT_FAILURE;
    }
	printf("changing the code,han 2019-2-17-1 \r\n");
	//printf("%s, %s, %s \n", argv[0], argv[1], argv[2]);
    list_in = argv[1];  //list_key.txt
    ratio = 0.6;       
    file_out = argv[2]; //matches_key.txt
    ...
    for (int i = 0; i < num_images; i++) {
		...
		//ÊÔÍ¼´òÓ¡
	//	std::cout << "han watch key[0] contains "<<std::endl;
		//
	/*	for (int iter = 0; iter < 100; iter++) {
			std::cout << keys[0][iter] << std::endl;
		}*/
