               ...
               std::vector<camera_params_t> &cameras)
{
	...
	fprintf(f_scr, "#changed by yixiaoHan,change data 2019-3-23\n");
	//实现地址的自动补完
	fprintf(f_scr, "#定义变量储存当前地址\n");
	fprintf(f_scr, "WORK_DIR=$PWD; \n");
	fprintf(f_scr, "cd ../../;\n");
	fprintf(f_scr, "cd bin; \n");
    //跳转到bin目录下，定义地址；
    fprintf(f_scr, "BUNDLER_BIN_PATH=$PWD; # Edit this line before running\n");
	fprintf(f_scr, "cd $WORK_DIR; \n");
	//回到原目录下
